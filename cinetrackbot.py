...
async def movie(ctx,*,mname):
    try:
        ...
        await ctx.reply(f"You havent given a proper movie name to search. Try Again!")
@bot.command()
async def rules(ctx):
...
async def movdesc(ctx,movid):
    try:
        ...
        await ctx.send(f"You havent given a movie id for me to provide description for :) {ctx.author.mention}")
@bot.command()
